(function (win, doc) {
    ...
    function connect() {
        ...
        ajax.send();
        ajax.addEventListener("readystatechange", responseConnection, false);
    }

    function responseConnection() {
    ...
    function writeItens(data) {
        for (var i = 0; i < data.items.length; i++) {
            createItem(data, i);
        }
    }

    function formatingGold(numero) {
    ...
    function createItem(data, i) {
        ...
        var valorTotal = formatingGold(data.items[i].product.price.value);
        var valorParcelas = formatingGold(data.items[i].product.price.installmentValue);
        ...
        if (data.items[i].product.images.length <= 1) {
            ...
            shopValueActualParcels <= 1 ?  $totalParcels.textContent = "R$: 0,00" : $totalParcels.textContent = `10 x de ${formatingGold(shopValueActualParcels)};`
            console.log(shopValueActualParcels);

            shopValueActualTotal <= 1 ? $totalValor.textContent = "R$: 0,00" : $totalValor.textContent = `ou ${formatingGold(shopValueActualTotal)} à vista`;
